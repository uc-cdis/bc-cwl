...
#################################################
#
#  Run this script in the same directory as your csv file of extracted subcortical information. All of the output will go into the same folder.
#
...
write("Structure\tNumberExcluded\tTotalGoodSubjs\tMean\tStandDev", file=args[2]);
...
for(x in seq(2, length(cnames)-2, 2)){
	...
	miss=which(interm=="");
	if(length(miss)>0){
		stop("There were missing values detected in your csv file of extracted subcortical information. Open your csv file \
		in Excel and locate any blank cells. Missing values should be marked with the letter x in the csv \
		file. \n\n");
	}
	...
	negs=which(interm<0);
	if(length(negs)>0){
		stop("Some of your volume values are negative. This does not makes sense. Open your csv file of extracted subcortical information \
	...
	write.table(t(as.matrix(stats)), file=args[2], append=T, quote=F, col.names=F,row.names=F,sep="\t");
